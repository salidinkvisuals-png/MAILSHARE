...
export default function InboxPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setAccount(accounts.data.find((a) => a.id === accountId));
  ...
  return (
    <div className="px-6 lg:px-10 py-10 max-w-[1400px]" data-testid="inbox-page">
      <Link to="/app/accounts" className="overline inline-flex items-center gap-1 hover:text-[#002FA7]">
      ...
      <div className="mt-2 animate-fade-up">
        ...
        <p className="text-neutral-600 mt-2 text-sm">{emails.length} emails · provider: <span className="font-mono uppercase">{account?.provider}</span></p>
      </div>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-px bg-neutral-200 border border-neutral-900">
        <div className="lg:col-span-5 bg-white max-h-[70vh] overflow-y-auto">
          <div className="overline px-4 py-3 border-b border-neutral-200 sticky top-0 bg-white">▸ MAIL LIST</div>
          {emails.map((e) => (
            <button
              key={e.id}
              onClick={() => setSelected(e)}
              data-testid={`email-row-${e.id}`}
              className={`w-full text-left px-4 py-3 border-b border-neutral-200 hover:bg-neutral-50 transition-colors ${selected?.id === e.id ? "bg-neutral-100" : ""}`}
            >
              <div className="flex justify-between items-baseline gap-2">
                <div className="font-mono text-xs text-neutral-700 truncate">{e.from_name || e.from_email}</div>
                <div className="font-mono text-[10px] text-neutral-500 shrink-0">{(e.received_at || "").slice(5, 10)}</div>
              </div>
